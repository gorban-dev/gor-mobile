...
This is the superpowers `brainstorming` skill, adapted for Android/gor-mobile. The skill text below is verbatim; the overlay only adds:

- **Architecture rules to load first** (always): `$HOME/.gor-mobile/rules/rules/core.md` and `$HOME/.gor-mobile/rules/rules/architecture.md`. Skim them before generating approaches so proposals conform to layer boundaries (Screen / View / ViewModel / UseCase / Repository / DataSource / DiModule).
...
9. **Transition to implementation** — invoke writing-plans skill (i.e. run `/plan`) to create implementation plan
...
- Invoke the writing-plans skill (run `/plan`) to create a detailed implementation plan
...
If visual questions are likely (mockups, layouts, diagrams), offer the visual companion once for consent, in its own message. If accepted, decide per-question whether the browser or terminal serves the user better.
