...
matching the changed files. An example *matches* when the diff carries a
declaration of the kind that example demonstrates — a new sealed action type
is matched by the layer's action and event examples, not by its ViewState
example; a layer's remaining files are not owed to a diff that declares
nothing of their kind. If your own check finds matching canonical
