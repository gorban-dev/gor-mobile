...
- UI bug / visual issue → `/test-ui`
- Final check before merge → `/verify`
- Wrap up branch / open PR → `/finishing-branch`

## Rules you enforce

1. **Delegation is MANDATORY** in `/implement`, `/tdd` (RED), routine `/debug`, `/review`, `/test-ui` (vision). The commands invoke `gor-mobile llm <role>` — do not bypass it.
