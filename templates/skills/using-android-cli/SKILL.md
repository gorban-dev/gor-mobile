...
`brainstorming` and `writing-plans` overlays plus the implementer tooling
contract in the `subagent-driven-development` skill gate on it per phase,
and the spec-/plan-document review prompts verify the citations — they
reference this section rather than restating the ladder.
